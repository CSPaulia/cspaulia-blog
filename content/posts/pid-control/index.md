...
<figure>
  <img src="pid-response-metrics.svg" alt="标明响应曲线、目标值、稳定带、稳态值、稳态误差、上升时间、超调量和调节时间的阶跃响应图">
  <figcaption>一次目标突变后的典型响应。绿色区域是目标值上下各 \(5\%\) 的示例稳定带；蓝色双向箭头分别标出各项时间或幅值指标。</figcaption>
</figure>

<details>
<summary><strong>展开：图中各名词分别表示什么？</strong></summary>

- **响应曲线 \(y(t)\)**：系统实际输出随时间的变化。图中红线就是响应曲线；在汽车定速例子中，它表示实际车速。
- **目标值 \(r(t)\)**：希望系统达到的输出。图中的蓝色虚线在目标突变后保持不变，因此这种输入称为阶跃输入。
- **稳定带（Tolerance Band）**：目标值附近允许存在的小范围误差，工程中常取目标值的 \(\pm2\%\) 或 \(\pm5\%\)。图中用绿色区域表示，并以 \(\pm5\%\) 为例。它是一段<strong>纵轴上的允许范围</strong>，不是某一段时间。
- **上升时间 \(t_r\)**：响应第一次从目标变化量的 \(10\%\) 上升到 \(90\%\) 所用的时间。上升时间越短，通常表示响应越快。
- **超调量（Overshoot）**：响应峰值超过目标值的部分。常用百分比表示为

  \[
  M_p=\frac{y_{\max}-r}{r}\times100\%.
  \]

- **调节时间 \(t_s\)**：从目标突变开始，到响应进入稳定带并且此后始终不再离开稳定带所需的时间。图中的竖直虚线标出了这个“最后一次进入”的时刻。
- **稳态值 \(y_{ss}\)**：瞬态振荡消失后，系统输出最终趋近的值。图中紫色虚线表示稳态值。
- **稳态误差 \(e_{ss}\)**：目标值与稳态值之间的<strong>纵向差值</strong>，而不是响应曲线末尾的一段，定义为

  \[
  e_{ss}=\lim_{t\to\infty}\bigl(r(t)-y(t)\bigr)=r-y_{ss}.
  \]

响应已经进入稳定带，只说明误差足够小且不再明显振荡，并不一定意味着稳态误差严格等于零。
